...
(async () => {
  ...
  await page.goto("https://www.insomniac.com/events/festivals/", {timeout: 0});
  ...
  const festivalsList = allFestivals.filter((festival) => !(festival["url"].includes("insomniac.com")) && !(festival["url"].includes("edsea")) && !(festival["url"].includes("holyship")) && !(festival["url"].includes("mexico")) && !(festival["url"].includes("hotel")));
  ...
  festivalsList.forEach(function(festival) {
    app.get('/api/festivals/'.concat(festival["name"].replaceAll(" ", "")), async function (req, res) {
      ...
      const lineup = await page.evaluate(() => 
        Array.from(document.querySelectorAll(".js-wp-template-Modal.no-barba"), (e) => e.textContent)
      );
      for (let i = 0; i < lineup.length; i++) {
        if (lineup[i].includes("Sunset Set") || lineup[i].includes("Live") || lineup[i].includes("Laser Harp") || lineup[i].includes("Tech Energy") || lineup[i].includes("Detox Set") || lineup[i].includes("DJ Set") || lineup[i].includes("DNB") || lineup[i].includes("Sunrise Set") || lineup[i].includes("House Set") || lineup[i].includes("Progressive House / Trance Set") || lineup[i].includes("Rabbit in the Moon") || lineup[i].includes("UK") || lineup[i].includes("Declan James & Decoder") || lineup[i].includes("DNB Set") || lineup[i].includes("Throwback Set") || lineup[i].includes("&ME, Rampa, Adam Port") || lineup[i].includes("Hybrid") || lineup[i].includes("of Miami") || lineup[i].includes("True Vine + Sister System")) {
      ...
      lineupNoDupe.sort(function(a,b) { return (a.toLowerCase() < b.toLowerCase()) ? -1 : 1;});
